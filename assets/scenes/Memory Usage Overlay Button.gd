...
export(int, "OFF", "ON") var memory_overlay = 0 setget set_memory_overlay
export(NodePath) var fnf_button_node
export(float) var focus_pos = 0.0 setget set_focus_pos

func set_memory_overlay(n_memory_overlay):
	memory_overlay = n_memory_overlay
	if not is_inside_tree(): yield(self, "ready")
	var text = ""
	match memory_overlay:
		0:
			text = "OFF"
		1:
			text = "ON"
...
func update_mem_overlay():
	if memory_overlay == 1:
		GameData.data.video_settings.mem_overlay = true
	else:
		GameData.data.video_settings.mem_overlay = false
...
func _ready():
	...
	if GameData.data.video_settings.mem_overlay:
		$Label.text = ":  ON"
	else:
		$Label.text = ":  OFF"
#	var test_dict = {
...
func _on_FNF_Button_gui_input(event):
	if event.is_action_pressed("ui_left"):
		self.memory_overlay = posmod(memory_overlay-1, 2)
