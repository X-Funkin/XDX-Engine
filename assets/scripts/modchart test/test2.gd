...
#	$"Note Track".load_chart()
	...
	get_tree().call_group("Note Track", "signal_self")
...
#			if note.index == closest_note.index:
#				pass
...
func _on_dupe_timer_timeout():
	var dupe_track = $"Note Track".duplicate(DUPLICATE_USE_INSTANCING)
	$dupe.add_child(dupe_track)
	pass # Replace with function body.
